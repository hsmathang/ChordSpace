...
- ¿Cuál es la sensibilidad del modelo a variaciones en estos parámetros? (Señalar que el repositorio permite sobreescritura; discutir en §3.8 Supuestos). (Nota de santimath: que permite ponert otros modelos de rugosidad pero que apra este rabajo nos quedamos con este jmmm y toca decir porque)
    ...
    > **Resolución (Q-016):** La sensibilidad del modelo debe distinguirse en dos niveles. A nivel **estadistico** (ajuste global a datos perceptuales), el modelo es robusto: Masina y Lo Presti \cite{MasinaLoPresti2024} demuestran que al variar la funcion de peso de los armonicos (de $1/n$ a $1/\beta^{n-1}$), los valores de chi-cuadrado no cambian significativamente, indicando que la prediccion general de consonancia es estable ante perturbaciones moderadas de los parametros. A nivel **topologico** (existencia y ubicacion de minimos locales), la sensibilidad reside primariamente en las **amplitudes** de los parciales, no en $A_1, A_2$. Mukherjee \cite{Mukherjee2023} formaliza esto: perturbaciones locales en la envolvente espectral pueden crear o eliminar singularidades incidentales, alterando la estructura de minimos de la curva de disonancia. Sin embargo, la **ubicacion** de las consonancias principales (unisono, octava, quinta) es robusta porque depende mas de la estructura espectral del timbre que de variaciones finas en la pendiente de la curva de rugosidad \cite{Sethares1993}. Respecto a modelos alternativos: Vassilakis logra una mejor correlacion con datos experimentales frente a HK \cite{Vassilakis2001}; el repositorio ChordSpace permite sobreescritura de parametros (arquitectura modular), y el paquete *hrep* implementa Sethares, HK y Vassilakis como backends intercambiables \cite{Harrison2020}. La eleccion pragmatica de Sethares con parametros estandar se justifica para el alcance exploratorio del trabajo actual, delegando comparaciones sistematicas como linea futura.
    <!-- FIN REDACTADO: Q-016 -->
...
- ¿El uso de $\log_2$ vs $\ln$ en JSD afecta la métrica? (Solo escala; la propiedad métrica de $\sqrt{\text{JSD}}$ se mantiene).
    <!-- REDACTADO: Q-027 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 5 afirmaciones → 3 Nivel A (InfoTheory/Math), 2 Nivel B -->
    <!-- Fase 2: Claim = "Cambio de base es solo factor de escala; no altera topología ni axiomas métricos." -->
    <!-- Fase 3: 3 argumentos (Propiedad logarítmica [1], Invarianza topológica [2], Convención visualización [1,3]) -->

    > **Resolución (Q-027):** El uso de logaritmos en base 2 (bits) frente a base natural (nats) en la definición de la Divergencia Jensen-Shannon introduce exclusivamente un **factor de escala multiplicativo constante** ($1/\ln 2 \approx 1.44$), derivado de la identidad $\log_2(x) \propto \ln(x)$ \cite{Shannon1948}. Matemáticamente, si $\sqrt{\text{JSD}_{nats}}$ es una métrica, entonces $\sqrt{\text{JSD}_{bits}} = \sqrt{1/\ln 2} \cdot \sqrt{\text{JSD}_{nats}}$ también lo es, pues multiplicar una métrica por una constante positiva preserva todos sus axiomas (desigualdad triangular, simetría, identidad) y no altera el ordenamiento de los vecinos más cercanos (topología) \cite{Endres2003}. ChordSpace adopta la convención de **bits** ($\log_2$) para reportar valores de entropía y divergencia, alineándose con la literatura de t-SNE y visualización de datos donde la perplejidad se interpreta como $2^H$ \cite{Maaten2008}.
...
-   ¿Es la matriz $D$ definida positiva / semidefinida? (Relevante para MDS clásico; no necesariamente para SMACOF).
        <!-- REDACTADO: Q-029 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
        <!-- Fase 1: ~12 afirmaciones evaluadas → 5 Nivel A (Math/Dimred), 4 Nivel B (Music/Psico), 3 Nivel C -->
        <!-- Fase 2: Claim central = "D perceptual no suele ser Euclidiana (Gram matrix no PSD); SMACOF es preferible matemáticamente." -->
        <!-- Fase 3: 3 argumentos (Condición PSD ×2 fuentes, SMACOF vs Strain ×3 fuentes, Naturaleza no-euclidiana ×3 fuentes) -->

        > **Resolución (Q-029):** Una matriz de distancias $D$ es *Euclidean embeddable* (representable perfectamente en $\mathbb{R}^k$) si y solo si su matriz de Gram derivada mediante doble centrado es semidefinida positiva (PSD) \cite{Burgoyne2011,Kim2018}. Sin embargo, las disimilitudes perceptuales y musicales frecuentemente violan los axiomas métricos (como la desigualdad triangular o la simetría) o generan matrices de Gram con valores propios negativos, lo que indica que el espacio subyacente es intrínsecamente no euclidiano \cite{Tymoczko2011,Krumhansl1990}.
        >
        > En este contexto, el MDS Clásico (*Strain*) no es adecuado, ya que su solución espectral requiere truncar los valores propios negativos, descartando información estructural o produciendo coordenadas complejas \cite{Delicado2020}. Por el contrario, la optimización iterativa de *Stress* (implementada via SMACOF) es matemáticamente preferible para espacios perceptuales, pues busca la mejor configuración $\mathbb{R}^2$ que preserva monotónicamente las disimilitudes, minimizando el error de ajuste incluso cuando un embedding perfecto es imposible \cite{Kraemer2018,Bernardes2016}.
...
- ¿Es el stress una medida suficiente o necesitamos métricas complementarias? (Nota de santimath: Excelnte pregunta hay que ver si otros trabajos de reprenstancion de acordes usan MDS y como lo evaluan)
    ...
    <!-- REDACTADO: Q-030 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 12 afirmaciones → 5 Nivel A (DimRed/Psych), 4 Nivel B, 3 Nivel C -->
    <!-- Fase 2: Claim = "MDS Métrico es la elección correcta porque rho es una escala de razón derivada físicamente; linealidad es deseable." -->
    <!-- Fase 3: 3 argumentos (Naturaleza de datos [1], Estructura global [6-8], Interpretabilidad de magnitud [11]) -->

    > **Resolución (Q-030):** La elección de **MDS métrico** (*Metric MDS*) frente al no-métrico se justifica porque la disimilitud $\rho$ en ChordSpace deriva de un modelo computacional (rugosidad psicoacústica) que produce una **escala de razón** con magnitudes significativas, no meramente rangos ordinales subjetivos \cite{Sethares1993, Borg2005}. Tratar estos valores como ordinales (MDS no-métrico) descartaría información crítica sobre *cuánto* más disonante es un acorde respecto a otro. Además, el MDS métrico minimiza el error cuadrático sobre las distancias ($\sum (d_{ij} - \delta_{ij})^2$), preservando mejor la **estructura global** y la geometría lineal del espacio, lo cual es esencial para interpretar trayectorias de modulación, a diferencia del no-métrico que permite distorsiones monótonas arbitrarias \cite{Cox2001, France2011}. Solo si el stress métrico resultara inaceptablemente alto ($>0.2$) se justificaría relajar la restricción lineal hacia una transformación monótona.

    <!-- FIN REDACTADO: Q-030 -->

    <!-- REDACTADO: Q-031 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 15 afirmaciones → 6 Nivel A (DimRed), 5 Nivel B, 4 Nivel C -->
    <!-- Fase 2: Claim = "Stress mide error global; OBLIGATORIO reportar métricas locales (Trustworthiness) para auditar topología." -->
    <!-- Fase 3: 3 argumentos (Insuficiencia del Stress [Venna], Métricas de Rango [Lee], Diagnóstico cruzado MDS/t-SNE [McInnes]) -->

    > **Resolución (Q-031):** El *Stress* de Kruskal cuantifica exclusivamente la fidelidad global de las distancias (error cuadrático), pero es **insuficiente** para diagnosticar fallas topológicas locales, como el colapso de vecindarios o la creación de falsos vecinos (*tearing/crushing*) \cite{Venna2006}. La literatura metodológica moderna exige complementar el stress con métricas basadas en rangos (*rank-based metrics*) que auditen la estructura local. Específicamente, se debe reportar **Trustworthiness** ($T(k)$) para medir la fiabilidad de los vecindarios visualizados (penalizando intrusiones de puntos lejanos) y **Continuity** ($C(k)$) para medir la preservación del manifold original (penalizando extrusiones) \cite{Venna2001, Lee2009}. Un buen stress con baja trustworthiness indica que el mapa preserva la geometría global a costa de falsificar relaciones locales significativas. El diagrama de Shepard sirve como diagnóstico visual de la linealidad del ajuste global \cite{Borg2005}.
...
- ¿Cómo interpretar un embedding UMAP donde las distancias globales no son significativas? (Nota de santimath:  no lo sé , pero me parece una pregunta importante y tecnica y deberiamos pensar en como resolverlo)
    ...
    <!-- REDACTADO: Q-032 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 10 afirmaciones → 4 Nivel A (DimRed), 3 Nivel B, 3 Nivel C -->
    <!-- Fase 2: Claim = "Coordenadas NO son comparables; usar métricas de rango (T/C) y correlaciones para comparar calidad estructural." -->
    <!-- Fase 3: 3 argumentos (Invarianza topológica [1], Matriz de Co-ranking [4], Procrustes para alineación [16]) -->

    > **Resolución (Q-032):** Los embeddings generados por MDS, UMAP y t-SNE **no son directamente comparables** en términos de sus coordenadas cartesianas o distancias euclidianas absolutas, debido a que optimizan funciones de costo fundamentalmente distintas (preservación de distancia global vs. divergencia de vecindarios locales) y poseen diferentes invarianzas de escala y rotación \cite{McInnes2018, Maaten2008}. La comparabilidad rigurosa se establece mediante **métricas de calidad estructural** agnósticas a la geometría absoluta: (1) **Matriz de Co-ranking** y sus derivados ($T(k)$, $C(k)$) para evaluar la fidelidad topológica; (2) **Correlación de Spearman** de distancias por pares para evaluar la preservación de la jerarquía global \cite{Lee2009, Espadoto2019}. Si se requiere una superposición visual estricta para comparar la disposición de clusters, es mandatorio aplicar primero un **Análisis de Procrustes** para alinear las soluciones espacialmente (traslación, rotación, escalado óptimos) \cite{Wang2021}.

    <!-- FIN REDACTADO: Q-032 -->

    <!-- REDACTADO: Q-033 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 12 afirmaciones → 5 Nivel A (UMAP/Topology), 4 Nivel B, 3 Nivel C -->
    <!-- Fase 2: Claim = "Proximidad local = Alta Probabilidad; Distancia Global = No fiable. Densidad es normalizada." -->
    <!-- Fase 3: 3 argumentos (Interpretación probabilística [1], Distorsión de densidad [7], Ejes sin significado [8]) -->

    > **Resolución (Q-033):** La interpretación de UMAP requiere cautela metodológica: la **proximidad local** en el gráfico implica una alta probabilidad de que los acordes sean vecinos en el espacio original (preservación toplógica confiable), pero las **distancias globales** entre clusters distantes no son métricamente fiables ni linealmente interpretables \cite{McInnes2018}. A diferencia de MDS, UMAP distorsiona el espacio vacío para separar variedades. Críticamente, UMAP aplica una normalización de densidad local (basada en el k-ésimo vecino), lo que significa que el **tamaño visual** de los clusters no refleja su dispersión real (varianza) en el espacio de entrada; regiones dispersas pueden aparecer tan compactas como regiones densas \cite{Coenen2019}. Los ejes X/Y carecen de significado intrínseco. Para validar afirmaciones sobre la estructura visualizada, se debe reportar el **Trustworthiness** ($>0.9$ idealmente) y evitar inferencias sobre relaciones de larga distancia que no estén corroboradas por MDS o correlaciones globales.
...
**Preguntas clave:**
- ¿Hay combinaciones inválidas? Sí: métricas de distribución (js, hellinger) con normalización `identity` requieren normalización L1 previa (documentar cómo el pipeline lo maneja).
    <!-- REDACTADO: Q-035 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 10 afirmaciones → 5 Nivel A (InfoTheory/ML), 3 Nivel B, 2 Nivel C -->
    <!-- Fase 2: Claim = "JSD/Hellinger exigen Simplex (L1); aplicar sobre 'identity' es matemáticamente inválido." -->
    <!-- Fase 3: 3 argumentos (Axiomas de divergencia [1], Requisito probabilístico [4], Práctica en scRNA-seq/ML [9]) -->

    > **Resolución (Q-035):** Existen **combinaciones matemáticamente inválidas** que deben ser excluidas o corregidas en el diseño experimental. Específicamente, las métricas basadas en teoría de la información como la **Divergencia de Jensen-Shannon (JSD)** y la **Distancia de Hellinger** están axiomáticamente definidas sobre distribuciones de probabilidad (vectores no negativos con norma $L_1=1$) \cite{Endres2003, Cha2007}. Aplicar JSD a vectores no normalizados (e.g., normalización `identity` con magnitudes arbitrarias) viola la definición de divergencia y produce resultados sin interpretación geométrica válida. Por tanto, el pipeline de ChordSpace implementa una regla de **coerción estricta**: si se selecciona una métrica probabilística, los datos deben ser proyectados al simplex previamente; de lo contrario, la combinación (e.g., `identity` + `jsd`) se marca como inválida y se descarta del análisis para evitar artefactos numéricos y sesgos de magnitud no controlados.

    <!-- FIN REDACTADO: Q-035 -->
- ¿Es necesario corregir por comparaciones múltiples al comparar 200 escenarios?
    <!-- REDACTADO: Q-036 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 15 afirmaciones evaluadas → 6 Nivel A (MIR/Estadística), 5 Nivel B, 4 Nivel C -->
    <!-- Fase 2: Claim = "Bonferroni no es necesario para selección de modelos (ranking); reportar estabilidad (semillas) es el estándar." -->
    <!-- Fase 3: 3 argumentos (Selección vs Inferencia [MIR], Estabilidad como métrica [DimRed], Validación [Music]) -->

    > **Resolución (Q-036):** En contextos de *model selection* y exploración de hiperparámetros (grid search), la aplicación estricta de correcciones por comparaciones múltiples (como Bonferroni o FDR) no es la práctica estándar, pues el objetivo es identificar configuraciones candidatas óptimas (ranking), no refutar hipótesis nulas individuales sobre efectos causales \cite{DeHaas2011, Micchi2020}. La literatura metodológica en MIR y aprendizaje automático recomienda, en su lugar, reportar la **estabilidad** de los resultados mediante agregación sobre múltiples corridas independientes (promedio $\pm$ desviación estándar sobre 5-10 semillas), lo que mitiga el riesgo de *overfitting* a una inicialización alevosa \cite{McInnes2018, Mabadeje2025}. Para comparar formalmente si un algoritmo supera a otro (e.g., MDS vs UMAP) más allá de la varianza estocástica, se utilizan pruebas no paramétricas globales como Friedman seguida de post-hoc de Nemenyi o Tukey HSD, pero no para cada celda de una grilla de 200 combinaciones \cite{DeHaas2013, Kury2025}. Por tanto, ChordSpace adopta el reporte de tablas Top-$K$ con métricas agregadas y análisis de sensibilidad, priorizando la reproducibilidad del ranking sobre la significancia estadística aislada.
...
- ¿Por qué $k = 3$? ¿Es sensible la evaluación a este parámetro? (Discutir en §3.8).
    <!-- REDACTADO: Q-039 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 14 afirmaciones → 5 Nivel A (DimRed), 4 Nivel B (MIR/Music), 5 Nivel C -->
    <!-- Fase 2: Claim = "k=3 justificable por vecindario neo-Riemanniano y práctica MIR; reportar curvas T(k)." -->
    <!-- Fase 3: 3 argumentos (Vecinos parsimónicos ×2, Empírico k=4 ×1, Curvas T(k) como estándar ×2) -->

    > **Resolución (Q-039):** La elección de $k=3$ se justifica tanto teórica como empíricamente. Desde la perspectiva de la teoría musical, cada tríada mayor o menor posee exactamente **tres** vecinos neo-Riemannianos parsimoniosos (P, L, R) que comparten 2 de 3 notas y minimizan el voice leading \cite{Tymoczko2006, Bernardes2016}; este vecindario es el más natural para evaluar si el embedding preserva las relaciones armónicas inmediatas. Empíricamente, Burgoyne y Saul fijan $k=4$ para MVU sobre espacios tonales, demostrando que vecindarios pequeños preservan el 98\% de la varianza con estructura local intacta \cite{Burgoyne2005}. Karystinaios y Widmer utilizan $k=3$ para sobremuestreo SMOTE en espacios latentes de acordes \cite{Karystinaios2022}. La práctica recomendada es reportar curvas $T(k)$ y $C(k)$ para un rango $k \in \{1, ..., 15\}$, complementando el valor puntual con la métrica escalar $Q_{local}$ que promedia sobre vecindarios pequeños \cite{Lee2009, Venna2005}.

    <!-- FIN REDACTADO: Q-039 -->
- ¿Trustworthiness y continuity son redundantes o complementarios?
    <!-- REDACTADO: Q-040 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 8 afirmaciones → 4 Nivel A (DimRed), 2 Nivel B (Math), 2 Nivel C -->
    <!-- Fase 2: Claim = "Son complementarias: T penaliza intrusiones, C penaliza extrusiones; el trade-off depende del método DR." -->
    <!-- Fase 3: 3 argumentos (Asimetría via co-ranking ×2, Trade-off MDS vs UMAP ×2, Divergencia por dim. intrínseca ×1) -->

    > **Resolución (Q-040):** Trustworthiness ($T$) y Continuity ($C$) son métricas **complementarias** que capturan errores asimétricos opuestos, derivables ambas de la *co-ranking matrix* \cite{Lee2009, Venna2005}. $T(k)$ penaliza **intrusiones** (falsos vecinos en el embedding que no lo eran en el espacio original), mientras que $C(k)$ penaliza **extrusiones** (vecinos originales que se pierden en el embedding). Este dualismo es analógicamente equivalente a precisión vs. recall \cite{Venna2005}. En la práctica, MDS/PCA tiende a producir alta $C$ pero baja $T$ (conserva continuidad global pero aplasta el manifold, creando falsos vecinos), mientras que UMAP/t-SNE prioriza alta $T$ a costa de menor $C$ (preserva vecindarios locales pero puede fragmentar conexiones de largo alcance) \cite{Wang2021, Chicco2026}. La divergencia entre ambas métricas se amplifica cuando la dimensionalidad intrínseca de los datos excede significativamente la del embedding (2D). Por tanto, ChordSpace reporta **ambas** métricas conjuntamente para diagnosticar si las diferencias entre métodos reflejan errores de intrusiones, extrusiones, o ambos.
...
2. **Modo determinista:** `n_jobs=1`, `mds_n_init=4`, seed fija. Prioriza reproducibilidad sobre velocidad. (Nota de santimath: la verdad nose mucho sobre esto pero siento que falta detalle y definciones para poder hablar de esto, en realdiad es relevante mencionarlo? que dice la literatura?)
    <!-- REDACTADO: Q-041 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 12 afirmaciones → 5 Nivel A (DimRed), 3 Nivel B (MIR/Math), 4 Nivel C -->
    <!-- Fase 2: Claim = "Modo determinista (seed+n_jobs=1) es buena práctica estándar; inicialización espectral preferible." -->
    <!-- Fase 3: 4 argumentos (Semilla necesaria pero no suficiente ×2, Inicialización espectral ×2, n_init insuficiente ×1, Control de entorno ×2) -->

    > **Resolución (Q-041):** Sí, es relevante y constituye una buena práctica metodológica documentada. La literatura sobre SMACOF y UMAP identifica la **inicialización** como el factor más determinante para la reproducibilidad, recomendando inicialización espectral o PCA en lugar de configuraciones puramente aleatorias \cite{Wang2021, Kobak2021}. La fijación de semillas (`seed`) es necesaria pero por sí sola insuficiente para garantizar calidad; se complementa con `n_jobs=1` para eliminar condiciones de carrera en el SGD estocástico de UMAP \cite{McInnes2018}. Respecto a `mds_n_init=4`, estudios empíricos con 1,000 inicializaciones muestran que el mínimo global de Stress se alcanza en solo el 3.9\%–22.3\% de los intentos \cite{Dzemyda2021}; por tanto, 4 inicializaciones no garantizan el óptimo global pero ofrecen un compromiso práctico razonable cuando se prioriza determinismo con tiempos de ejecución acotados. ChordSpace mitiga esta limitación reportando la variabilidad inter-semillas (media $\pm$ desviación estándar) y utilizando distancia de Procrustes para cuantificar la estabilidad de los layouts \cite{McInnes2018, Tomo2025}.
...
- ¿La rugosidad de Sethares realmente captura "similitud sonora"? Es un proxy, no una medida directa.( Nota de santimath: hay mcuhas refernecia para jsutificar estas cosas)
    <!-- REDACTADO: Q-042 | Fuente: 5 notebooks NotebookLM | Workflow: /redactor-critico v2 | Fecha: 2026-02-17 -->
    <!-- Fase 1: 18 afirmaciones → 7 Nivel A (Psico/Music), 6 Nivel B (Armonía), 5 Nivel C -->
    <!-- Fase 2: Claim = "Roughness mide interferencia acústica (sensorial), NO similitud musical completa; proxy defendible si se delimita." -->
    <!-- Fase 3: 4 argumentos (Constructo sensorial ×3, Fallo predictivo en tríadas ×2, Multidimensionalidad ×3, Proxy defendible ×2) -->

    > **Resolución (Q-042):** La rugosidad (*roughness*) mide exclusivamente la **interferencia acústica** generada por batidos rápidos entre parciales dentro de la banda crítica auditiva \cite{Plomp1965, Sethares1993}; es un indicador de disonancia *sensorial* (procesamiento periférico/ascendente), no de similitud musical global. Sus limitaciones documentadas incluyen: (i) **dependencia tímbrica**, pues la curva de disonancia cambia con el espectro de los parciales \cite{Sethares1993, Tind2004}; (ii) **sensibilidad al registro**, ya que la rugosidad aumenta en registros graves por el ensanchamiento relativo de las bandas críticas \cite{Eerola2022}; (iii) **fallo predictivo en tríadas**, dado que modelos de rugosidad pura predicen erróneamente que la tríada aumentada es más consonante que la disminuida \cite{Cook2009, Masina2024}; y (iv) **omisión de factores cognitivos** como la harmonicidad, la familiaridad cultural y el contexto tonal \cite{Harrison2020, McDermott2016}. No obstante, como proxy es **defendible** dentro del alcance declarado de ChordSpace (similitud por disonancia sensorial, no consonancia musical completa), siempre que se delimite explícitamente que el constructo medido es la interferencia acústica interválica y se reconozca la necesidad futura de modelos combinados (rugosidad + compacidad) para una validez ecológica más amplia \cite{Masina2024, Stolzenburg2015}.
